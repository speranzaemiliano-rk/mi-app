...
function crearFutbolista(colCamisetaA, colCamisetaB, colShort, colMedias, numero, colPiel, colPelo){
  ...
  const pecho = tronco([
    ...
    [ 0.105, 0.146], [ 0.180, 0.152], [ 0.270, 0.170], [ 0.360, 0.183],
    [ 0.450, 0.185], [ 0.515, 0.174], [ 0.558, 0.148], [ 0.588, 0.104],
  ...
  cuello.add(nariz);
  [-1, 1].forEach(l => {
    const oreja = esf(0.026, matPiel);
    oreja.scale.set(0.35, 1, 0.7);
    oreja.position.set(l*0.098, 0.150, -0.006);
    cuello.add(oreja);
    ...
    ceja.position.set(l*0.038, 0.184, -0.088);
    ceja.rotation.z = -l*0.12;
    cuello.add(ceja);
  });
  // pelo: casquete que sigue el cráneo y baja en la nuca
  // El pelo es un casquete que arranca por encima de las cejas y baja en la
  // nuca. Cubriendo media esfera entera tapaba la cara.
  const pelo = new THREE.Mesh(
    new THREE.SphereGeometry(0.109, 22, 16, 0, Math.PI*2, 0, Math.PI*0.48), matPelo);
  pelo.scale.set(0.97, 1.10, 1.03);
  pelo.position.y = 0.150;
  pelo.rotation.x = -0.22;                       // más alto adelante que atrás
  pelo.castShadow = true;
  cuello.add(pelo);
  const nuca = esf(0.082, matPelo);
  nuca.scale.set(0.94, 0.88, 0.58);
  nuca.position.set(0, 0.142, 0.056);
  cuello.add(nuca);
  ...
  [-1, 1].forEach(lado => {
    ...
    hombro.position.set(lado*0.212, 0.505, 0);
    torso.add(hombro);
    const deltoide = esf(0.057, matManga);          // tapa la unión con el torso
    deltoide.scale.set(1.02, 0.86, 0.94);
    hombro.add(deltoide);
    ...
    hombro.add(codo);
    const rotula = esf(0.035, matPiel);
    rotula.scale.set(1, 0.72, 0.95);
    codo.add(rotula);
  ...
  [-1, 1].forEach(lado => {
    ...
    caderaP.add(rodilla);
    const rot = esf(0.058, matMedias);
    rot.scale.set(1.0, 0.62, 0.95);
    rot.position.y = -0.012;
    rodilla.add(rot);
    const bordeMedia = miembro([[0, 0.070], [1, 0.068]], 0.05, matMedias, 1, 0.95);
    bordeMedia.position.y = 0.030;
    rodilla.add(bordeMedia);
    // gemelo: la panza va arriba, es lo que le da forma a la pierna
    const pant = miembro([[0, 0.068], [0.22, 0.072], [0.55, 0.058], [1, 0.036]],
                         0.40, matMedias, 1, 0.95);
    rodilla.add(pant);
