...
**Remuneraciones.** En el menú lateral es un **título principal con submenú** (Legajos · Liquidar sueldo · Pagos y Consolidaciones · Resumen y Recibos); cada ítem muestra una vista del panel (`remVista`/`_remAplicarVista`, la última queda en `localStorage rk_rem_vista`). Cada concepto de la liquidación tiene dos ejes **independientes** además del signo (`_remNormConcepto`): `remun` (true = remunerativo, tributa los aportes de ley del 17%; false = bono/gratificación sin aportes) y `enRecibo` (true = figura en el recibo oficial; false = se liquida por fuera). El formato viejo (`tipo:'fuera'`) se normaliza al leer, no hay migración de datos. `remTotalesConceptos` devuelve `haberes` (remunerativos en recibo, base de aportes), `noRemun`, `descuentos`, `fueraHab`/`fueraDesc`/`fuera`. Dos netos: `remNetoOficial` (lo que sale impreso en el recibo) y `remNetoEmpleado` (total a cobrar = oficial + fuera). El recibo guardado (`remunRecibos`) graba `neto` (oficial) y `netoTotal`, y al cargar el pago el neto oficial va a «banco» y lo de afuera a «no oficial».
