...
- **Asistente RK** (chat flotante, `Google Gemini` con *function calling*): responde dudas sobre la app, analiza PDFs de facturas adjuntos (con botón para precargarlas en Facturas), consulta bajo demanda los datos reales del proyecto activo (presupuestos con pagos, facturas, ingresos, caja, cuentas bancarias, tareas pendientes del pizarrón, plan de trabajo, etc. — p. ej. "¿cuánto aportó Juan Pérez?" o "¿qué tareas tiene pendientes María?") haciendo cálculos y comparaciones, busca cualquier texto en todas las secciones a la vez — incluso en los demás proyectos y empresas ("¿dónde está la factura de Pinturería Central?") —, puede navegar la app ("llevame a Facturas") y **precargar formularios** desde el chat (p. ej. "cargá un pago de $500.000 a Constructora Sur"): abre el formulario con los campos completados y el usuario siempre revisa y guarda — el asistente nunca escribe directo en la base.
