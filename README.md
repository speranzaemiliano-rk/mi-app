...
- **Sincronizar entre equipos (opcional), con login.** Botón ☁ del encabezado: se entra con mail y contraseña y los datos espejan a `parteObra/<obraId>` en la Realtime Database. La sesión queda abierta en el equipo, así que se entra una sola vez. El `localStorage` sigue siendo la fuente local: la planilla abre y se usa sin señal, y sincroniza cuando hay. El SDK de Firebase se descarga **sólo si hay configuración** (sin configurar, la página no pide nada a la red).
  ...
  - **Tener cuenta no alcanza**: las reglas comparan `auth.token.email` contra la lista de habilitados, que vive **dentro de las propias reglas** (no en un nodo de datos, para no depender de crear un nodo anidado a mano). Las cuentas se crean a mano en Authentication → Users; no hay alta pública.
  - ⚠️ **Requiere dos pasos a mano en ese proyecto**: habilitar "Correo electrónico/contraseña" en Authentication, y publicar las reglas con los mails habilitados. Todo está documentado y listo para copiar en los comentarios de `obra/config.js`. Si falta algo, el botón ☁ lo dice con todas las letras en vez de fallar en silencio.
  - Sin entrar, la planilla funciona igual pero los datos quedan sólo en ese equipo. Quien abra la URL pública sin cuenta no ve nada de la obra.
- **Presentación de la marca** al abrir: el logo de Mess sobre el fondo oscuro de la identidad (ver `MARCA.md`), igual que el sistema. Se va sola en 1,5 s y se saltea tocando la pantalla. La salida es una animación CSS con `forwards`, así que aunque el JS falle no queda tapando la planilla.
- **Instalable en PC y en iPhone**: botón «📲 Instalar en este equipo» en el encabezado. En Chrome/Edge dispara el diálogo nativo (`beforeinstallprompt`); en iPhone, donde Safari no ofrece ninguno, explica el camino de Compartir → «Agregar a pantalla de inicio». Se esconde si ya está instalada (`display-mode: standalone`) o si la descartan.
- **Exportar a PDF** (botón «Días» → «Exportar a PDF»): antes de exportar se elige **qué días** (el día abierto, un rango de fechas, o todos), **qué gremios**, **qué datos** de cada persona (tarea, si vino, si tiene ART, CUIL) y si entran los ausentes. Un contador va diciendo cuántos días y líneas van a salir. Sale por la **impresión del navegador** («Guardar como PDF»), no por una librería: así no hay que descargar nada, funciona sin señal y el texto del PDF queda seleccionable. El reporte se arma en un `#reporte` oculto que sólo aparece con `@media print`, con encabezado de marca (logo Mess), resumen del día, una tabla por gremio con numeración y filas alternadas, los ausentes tachados, una caja de alerta con quién estuvo sin ART, y líneas de firma (capataz / jefe de obra / fecha) cuando el informe es de un solo día.
...
- **Personal** (botón «Personal»): la lista de gente, fija y reutilizable todos los días. Por persona: gremio (selector), tarea, y si tiene ART o no. Tiene **buscador** (nombre, gremio, tarea o CUIL; sin tildes y por varias palabras, así «cubas rojas claudio» trae uno solo entre los apellidos repetidos) que se combina con los filtros «sin gremio» y «sin ART». Se puede agregar gente a mano y borrar.
...
- **Responsable por pendiente**: se le asigna a alguien del directorio (nombre y mail, se carga una vez y queda). Sale en el chip del ítem, en el texto de WhatsApp y en el informe.
- **Quién controló**: al tildar queda registrado quién lo dio por hecho y cuándo. Con sesión abierta va el nombre de la cuenta; sin sesión, «este equipo». Al destildar se limpia.
- **Fotos de lo terminado**: desde el pendiente (📷) o desde el botón «📷 Foto» de arriba, que **pregunta a qué unidad, rubro y pendiente** asignarla. Se comprimen en el teléfono antes de guardar, y los bytes se bajan **sólo cuando alguien abre la foto** — abrir el tablero no se baja todas las fotos del edificio.
- **Compartir por WhatsApp**: desde una unidad o desde un rubro, arma el texto con lo que falta (lo tildado no sale, y cada línea lleva su responsable) y lo manda o lo copia.
- **Asistente (✨ abajo a la derecha)**: se le dicta o se le escribe («en la 101 ya pintaron la segunda mano», «agregá en la 204 que falta el zócalo del baño», «lo de durlock es de Juan») y **propone** los cambios con casilleros para confirmar. Nunca aplica nada solo: dictando en obra la transcripción falla seguido, y dar por hecha una unidad equivocada es peor que no tener asistente. El micrófono tiene dos caminos según el navegador: donde hay dictado propio transcribe al instante y sin gastar datos; donde no (Firefox, iPhone fuera de Safari, Brave) **graba y le manda el audio**. Se le puede dictar una lista de varias tareas de una y las carga todas.
