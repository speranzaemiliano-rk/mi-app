...
- **Sincronizar entre equipos (opcional).** Con el botón ☁ del encabezado se genera un **código de 24 caracteres** que viaja en el link (`?o=<codigo>`); quien abra ese link ve y edita la misma planilla. Los datos van a `parteObra/<codigo>` en la Realtime Database. El `localStorage` sigue siendo la fuente local, así que la planilla funciona sin señal y sincroniza cuando hay; el SDK de Firebase se descarga **sólo si hay código** (sin sincronizar, la página no pide nada a la red).
  - ⚠️ **El link es la única llave**: no hay login, así que cualquiera que lo consiga lee y escribe la nómina completa, nombres y CUIL incluidos. Es una decisión consciente de simplicidad sobre seguridad; si hace falta cerrarlo, hay que pasar a login con rol.
  - **Usa un proyecto de Firebase SEPARADO del sistema de gestión**, configurado en `obra/config.js` (apiKey, databaseURL, projectId — valores públicos). El nodo de la obra es de lectura y escritura abierta para quien tenga el código, así que aislarlo evita que un error en esas reglas pueda tocar `empresas`, `global` o `remuneraciones`. Por eso la regla `parteObra` **no** está en `database.rules.json`: va publicada en el proyecto de la obra, y su texto está comentado dentro de `obra/config.js`.
  - ⚠️ **Requiere publicar la regla `parteObra` a mano** en ese proyecto (Realtime Database → Reglas). Concede lectura/escritura sólo al nodo `parteObra/<codigo>` con código de 20+ caracteres, y no permite listar `parteObra`, así que no se pueden descubrir los códigos de otras obras.
  - La base configurada es el proyecto **`control-caja-965ad`**, aparte del sistema. Alcanza con la `databaseURL`: la `apiKey` sólo haría falta si algún día se agrega login. Si `obra/config.js` queda sin `databaseURL`, la planilla funciona igual (guarda en el equipo) y el botón ☁ avisa que falta configurar la base.
