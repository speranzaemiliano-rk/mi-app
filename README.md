...
- **Sincronizar entre equipos (opcional), con login.** Botón ☁ del encabezado: se entra con mail y contraseña y los datos espejan a `parteObra/<obraId>` en la Realtime Database. La sesión queda abierta en el equipo, así que se entra una sola vez. El `localStorage` sigue siendo la fuente local: la planilla abre y se usa sin señal, y sincroniza cuando hay. El SDK de Firebase se descarga **sólo si hay configuración** (sin configurar, la página no pide nada a la red).
  ...
  - **Tener cuenta no alcanza**: las reglas exigen además una entrada en `permitidos/<uid>`, nodo sin `.write` que sólo se edita desde la consola de Firebase. Las cuentas se crean a mano en Authentication → Users; no hay alta pública.
  - ⚠️ **Requiere tres pasos a mano en ese proyecto**: habilitar "Correo electrónico/contraseña" en Authentication, publicar las reglas, y dar de alta cada usuario en `permitidos`. Todo está documentado y listo para copiar en los comentarios de `obra/config.js`. Si falta algo, el botón ☁ lo dice con todas las letras en vez de fallar en silencio.
